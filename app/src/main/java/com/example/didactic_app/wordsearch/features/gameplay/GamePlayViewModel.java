package com.example.didactic_app.wordsearch.features.gameplay;
...
import com.example.didactic_app.wordsearch.commons.SingleLiveEvent;
import com.example.didactic_app.wordsearch.commons.Timer;
...
import com.example.didactic_app.wordsearch.data.entity.GameDataMapper;
import com.example.didactic_app.wordsearch.model.GameData;
import com.example.didactic_app.wordsearch.model.UsedWord;
...
public class GamePlayViewModel extends ViewModel {
    ...
    public void generateNewGameRound(int rowCount, int colCount, String palabras) {
        if (!(mCurrentState instanceof Generating)) {
            setGameState(new Generating(rowCount, colCount, "Play me"));

            Observable.create((ObservableOnSubscribe<GameData>) emitter -> {
                List<Word> wordList = mWordDataSource.getWords(palabras);
